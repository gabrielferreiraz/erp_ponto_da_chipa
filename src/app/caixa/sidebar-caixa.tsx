...
export function SidebarCaixa({ user }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside 
        className={cn(
          "bg-white border-r border-zinc-200/60 flex flex-col flex-shrink-0 relative z-40 shadow-[4px_0_24px_rgba(0,0,0,0.02)] transition-all duration-300 ease-in-out",
      ...
      >
        ...
        <div className={cn(
          ...
          <div className="flex items-center gap-4 group cursor-default overflow-hidden">
            ...
            <div className={cn(
              ...
              isCollapsed ? "opacity-0 -translate-x-4 pointer-events-none" : "opacity-100"
            )}>
              ...
              >
                <div className="flex items-center gap-4 min-w-0">
                  <item.icon className={cn(
                    ...
                    isActive ? "text-[#E24A07]" : "group-hover:text-[#E24A07]"
                  )} strokeWidth={1.5} />
                  <span className={cn(
                ...
                {!isCollapsed && (
                  <ChevronRight className={cn(
                    "w-4 h-4 transition-all text-zinc-300",
        ...
        {/* User & Logout */}
        <div className={cn(
          "p-4 mt-auto border-t border-zinc-100/80 bg-zinc-50/30 transition-all duration-300",
          isCollapsed ? "p-4" : "p-6"
        )}>
          <div className={cn(
            "flex items-center justify-between gap-3 p-3 rounded-3xl bg-white border border-zinc-200/50 shadow-sm ring-1 ring-zinc-950/[0.02] transition-all duration-300 overflow-hidden",
          ...
          {isCollapsed && (
            <form action={logoutAction} className="mt-2">
              <button 
                type="submit"
                className="w-full flex justify-center p-3 rounded-2xl text-zinc-400 hover:text-red-600 hover:bg-red-50 transition-all active:scale-90"
                title="Sair"
              >
                <LogOut className="w-5 h-5" strokeWidth={2} />
              </button>
