...
export const GET = withSecurity(async (request) => {
  const { inicio, fim, periodo, categoriaId } = getParams(new URL(request.url))
  return NextResponse.json(await service.getRankingProdutos(inicio, fim, periodo, categoriaId))
}, { roles: ['ADMIN'] })
